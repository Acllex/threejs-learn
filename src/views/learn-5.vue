    ...
    (val) => {
      ...
      function transition() {
        for (let i = 0; i < bufArrays[0].length; i++) {
          ...
          item.to({position:THREE.MathUtils.randFloat(bufArrays[0][i]-0.03, bufArrays[0][i]+0.03)}, THREE.MathUtils.randFloat(1000, 4000)).onUpdate(({position}:{position:number})=> {
